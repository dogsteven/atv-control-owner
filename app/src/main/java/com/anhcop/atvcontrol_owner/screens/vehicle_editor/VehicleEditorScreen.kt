...
fun VehicleEditorScreen(
    ...
    val viewModel = hiltViewModel(
        creationCallback = { factory: VehicleEditorViewModelFactory ->
            factory.create(id)
        }
    )
    ...
    val isMacAddressValid = viewModel.isMacAddressValid
    val isLocalIPValid = viewModel.isLocalIPValid
    val isPriceValid = viewModel.isPriceValid
                    ...
                    title = {
                        Text(
                            text = when (editorMode) {
                                EditorMode.Add -> "Thêm xe mới"
                                EditorMode.Update -> "Cập nhật xe"
                            }
                        )
                    },
                    actions = {
            ...
            LazyColumn(
                ...
                item {
                    TextField(
                        ...
                        supportingText = if (!isNameValid) {
                            {
                                Text(
                                    text = "Biệt danh không được để trống",
                ...
                item {
                    TextField(
                        ...
                        supportingText = if (!isMacAddressValid) {
                            {
                                Text(
                                    text = "Địa chỉ Mac phải có dạng xx:xx:xx:xx:xx:xx",
                                    style = MaterialTheme.typography.labelMedium,
                                    color = MaterialTheme.colorScheme.error
                                )
                            }
                ...
                item {
                    TextField(
                        ...
                        onValueChange = { viewModel.localIP = it },
                        singleLine = true,
                        minLines = 1,
                        maxLines = 1,
                        isError = !isLocalIPValid,
                        supportingText = if (!isLocalIPValid) {
                            {
                                Text(
                                    text = "Địa chỉ IP cục bộ phải ở dạng xxx.xxx.xxx.xxx",
                                    style = MaterialTheme.typography.labelMedium,
                                    color = MaterialTheme.colorScheme.error
                                )
                            }
                ...
                item {
                    TextField(
                        ...
                        supportingText = if (!isPriceValid) {
                            {
                                Text(
                                    text = "Giá vé phải là một số dương",
                                    style = MaterialTheme.typography.labelMedium,
                                    color = MaterialTheme.colorScheme.error
                                )
                            }
                ...
                item {
                    Button(
                        onClick = {
                            viewModel.submit(
                                onAdded = {
                                    Toast.makeText(context, "Đã thêm xe thành công", Toast.LENGTH_SHORT).show()
                                },
                                onUpdated = {
